...
use chrono::Utc;
use shielder_scheduler_common::protocol::MerklePath;
...
use crate::{
    ...
    prepare_relay_calldata, AppState,
};
...
impl SchedulerProcessor {
    ...
    async fn process_request_logic(
        ...
        let relayer_address = Address::ZERO;

        // 3. Prepare the relay calldata using the TEE
        let res = prepare_relay_calldata(
            axum::extract::State(self.app_state.clone()),
            request.encryption_envelope,
            relayer_fee,
            relayer_address,
            merkle_path,
        )
        ...
        info!("Relay calldata prepared: {:?}", res);

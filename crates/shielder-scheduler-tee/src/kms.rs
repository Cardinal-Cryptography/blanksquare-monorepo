...
use shielder_scheduler_common::protocol::{AwsConfig, EncryptionEnvelope, TeeError};
...
impl KmsDecryptionController {
    pub fn new(
        ...
        Self {
            ...
            private_key: BASE64
                .decode(private_key)
                .expect("Failed to decode PRIVATE_KEY_BASE64 from base64"),
        }
    }
    ...
    fn decrypt_dek(
        ...
        let decrypted_data = {
            ...
            let stdout_str = String::from_utf8(output.stdout)
                .map_err(|_| TeeError::KMS("stdout not valid utf8".into()))?;
            ...
            let plaintext_line = stdout_str
                ...
                .ok_or_else(|| TeeError::KMS("No PLAINTEXT line found in kmstool output".into()))?;

            let b64_str = plaintext_line
                .strip_prefix("PLAINTEXT: ")
                .ok_or_else(|| TeeError::KMS("Invalid PLAINTEXT format".into()))?;

            info!("Parsing kmstool output success");
            BASE64
                .decode(b64_str.trim())
                .map_err(|_| TeeError::KMS("base64 decode failed".into()))?
        };
    ...
    pub fn verify_public_key(&self, aws_config: &AwsConfig) -> Result<(), TeeError> {

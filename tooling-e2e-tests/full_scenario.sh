...
scenario() {
  ...
  ${1} new-account $(mtzero 200) --memo $(referral_memo)
  ${1} deposit $(mtzero 100) --memo $(referral_memo)
  log_progress "✅ Native account created and deposited to it"

  ${1} new-account-erc20 $(mtzero 200) "${ERC20_CONTRACT_ADDRESS_1}" --memo $(referral_memo)
  ${1} deposit-erc20 $(mtzero 100) "${ERC20_CONTRACT_ADDRESS_1}" --memo $(referral_memo)
  ...
  ${1} withdraw $withdrawal_amount "${WITHDRAWAL_PUBLIC_KEY}" --memo $(referral_memo)
  ...
  ${1} withdraw-erc20 $withdrawal_amount "${WITHDRAWAL_PUBLIC_KEY}" "${ERC20_CONTRACT_ADDRESS_1}" $pocket_money --memo $(referral_memo)
...
run() {
  ...
  log_progress "🔄 Scenario with non-zero protocol fees"
  log_progress "🔄 Setting non-zero protocol fees..."
